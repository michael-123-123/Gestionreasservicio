...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initializeApp() {
        ...
        if (session && session.user) {
            // Consultar rol explícitamente para asegurar redirección correcta
            const { data: userProfile } = await db.from('users').select('role').eq('email', session.user.email).single();
            const userRole = userProfile ? userProfile.role : 'auxiliar'; 

            // Si NO es administrador (es auxiliar o sin rol), cargamos la interfaz
